...
RT_QC_radiometry <- function(PRES, IRR_380, IRR_412, IRR_490, PAR) {
  ...
  ##################################################################################################
  # This program was designed in order to perform the RT-QC for radiometry profiles acquired by 
  # Bio-Argo floats
  #
  ...
  #
  # 1: DARK IDENTIFICATION   
  # 2a:MAJOR CLOUDS IDENTIFICATION  
  # 2b:SWINGING PROFILE IDENTIFICATION 
  # 3:TYPE PROFILE IDENTIFICATION 
  # 4a:TYPE2_FLAG ASSIGNEMENT 
  # 4b:TYPE1_FLAG ASSIGNEMENT
  #
  ...
  ##################################################
  TAB_complete = TAB[complete.cases(TAB$IRR_380),]
  #to remove rows with NA. Same rows for any irradiance and PAR measurements.
  
  TAB_NA = TAB[!complete.cases(TAB$IRR_380),]
  #dataframe with only NA values. Useful when assignin flags.
  ...
  FLAG_NAMES = list("IRR_380"="FLAG_380_QC", "IRR_412"="FLAG_412_QC", 
                    "IRR_490"="FLAG_490_QC", "PAR"="FLAG_PAR_QC")
  ...
  fill_flag_3 <- function(param_name) {
    ...
    FLAG_param_QC = rep("3", length(TAB_complete[[param_name]])) 
    #ASSIGNEMENT CORRESPONDING to STEP 3

    TAB_complete[[ FLAG_NAMES[[param_name]] ]] <<- FLAG_param_QC
    TAB_NA[[ FLAG_NAMES[[param_name]] ]] <<- rep("NA", length(TAB_NA$PRES))
    
    TAB_param = rbind(TAB_complete, TAB_NA) 
    #to create a string of FLAG with same length that NETCDF file

    newdata_All[[ FLAG_NAMES[[param_name]] ]] <<-
      TAB_param[order(as.numeric(row.names(TAB_380))),][[ FLAG_NAMES[[param_name]] ]]  
    #to sort for the initial row order, NA included
  }
  ...
  for (param_name in PARAM_NAMES) {
    ...
    for (l in 1 :(length(TAB_complete$IRR_380)-4)) {
      Lilliefors_param[l] = 
        lillie.test(TAB_complete[[param_name]][ l:length(TAB_complete[[param_name]]) ])[2] 
      #[2] to select p-value
    }
    ...
    #flag4_param = pol_param_1$residuals < (mean_param-lim_sd3_param) | 
    #              pol_param_1$residuals > (mean_param+lim_sd3_param) # FLAG 4 points in STEP 2
    #pts.flag4_param = which(flag4_param) # FLAG 4 points in STEP 2
      
    rsquared_param = summary(pol_param_1)$r.squared
    #END of STEP 2a #REMEMBER! IN THE CODE THE CORRESPONDING STEP 2a FLAGS ARE ASSIGNED TOGETHER 
    #WITH THE FLAGS ASSIGNED DURING FOLLOWING STEPS  
    ...
    flag3plus_param = pol_param_1$residuals[i_depth] > (mean_param + lim_sd2_param)  
    #for checking sunny points during ovecast profiles below the surface 

    pts.flag3plus_param = which(flag3plus_param) 
    #for checking sunny points during ovecast profiles below the surface 
    
    flag3minus_param = pol_param_1$residuals[i_depth] < (mean_param - lim_sd2_param) 
    #for checking sunny points during ovecast profiles  below the surface
    
    pts.flag3minus_param = which(flag3minus_param) 
    #for checking sunny points during ovecast profiles below the surface
      
    # STEP 2b:SWINGING PROFILE IDENTIFICATION
    if ( (length(pts.flag3plus_param) > length(pts.flag3minus_param)) 
          & (rsquared_param < swinging_thresh) ) { 
      fill_flag_3(param_name)
      typeAll[[param_name]] = "3" #catherine
      next
    }
    ...
    pol_param_2 = lm(log(TAB_complete[[param_name]][no.cloudy_param]) ~ 
                       TAB_complete$PRES[no.cloudy_param] + 
                       I(TAB_complete$PRES[no.cloudy_param]^2) + 
                       I(TAB_complete$PRES[no.cloudy_param]^3) + 
                       I(TAB_complete$PRES[no.cloudy_param]^4)) 
    ...
    flag2_param_ADJ = pol_param_3$residuals < (mean_param_3 - lim_sd1_param_3) | 
                      pol_param_3$residuals > (mean_param_3 + lim_sd1_param_3) 
                    #FLAG 2 points in STEP 4b
    ...
    flag3_param_ADJ = pol_param_3$residuals < (mean_param_3 - lim_sd2_param_3) | 
                      pol_param_3$residuals > (mean_param_3 + lim_sd2_param_3) 
                    #FLAG 3 points in STEP 4b
    ...
    if (rsquared_param_2 > thresholds[[param_name]][1] & 
        rsquared_param_2 <= thresholds[[param_name]][2]) { 
      #FLAG ASSIGNMENT CORRESPONDING TO STEP 3
      FLAG_param_QC = rep("3", length(TAB_complete[[param_name]])) 
      #stop analysis #dark values are still assigned to FLAG 3 from STEP 1
      FLAG_param_QC[pts.flag3_param] = "3" 
      #FLAG 3 pts assigned in STEP 2a 
      #no-useful command but it is kept just to remind the STEP of assignement
      #FLAG_param_QC[pts.flag4_param]="3" #FLAG 4 pts assigned in STEP 2a
            
      typeAll[[param_name]] = "3" #catherine
    } else if (rsquared_param_2 > thresholds[[param_name]][2] & 
               rsquared_param_2 <= thresholds[[param_name]][3]) { 
      #begin of STEP 4a: TYPE2_FLAG ASSIGNEMENT
      
      FLAG_param_QC = rep("2", length(TAB_complete[[param_name]])) 
      # It is a profile without FLAGS 1 # assignment corresponding to STEP 3
      ...
      FLAG_param_QC = rep("1", length(TAB_complete[[param_name]])) 
      #ASSIGNEMENT CORRESPONDING to STEP 3
    ...
    newdata_All[[ FLAG_NAMES[[param_name]] ]] = 
      TAB_param[order(as.numeric(row.names(TAB_param))),][[ FLAG_NAMES[[param_name]] ]]  
    #to sort for the initial row order, NA included
    
    # newdata_All[[ FLAG_NAMES[[param_name]] ]] is the right string of characters to be included 
    # in NETCDF file
  }

  return(list("FLAG_380"=newdata_All$FLAG_380_QC, "FLAG_412"=newdata_All$FLAG_412_QC, 
              "FLAG_490"=newdata_All$FLAG_490_QC, "FLAG_PAR"=newdata_All$FLAG_PAR_QC, 
              "type380"=typeAll[["IRR_380"]], "type412"=typeAll[["IRR_412"]], 
