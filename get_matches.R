...
get_Ts_match <- function(path_to_netcdf, file_name, PARAM_NAME, material="PEEK", 
  ...
  TEMP_QC = unlist(str_split(ncvar_get(filenc_C, "TEMP_QC", 
                                       start=c(1,id_prof_C), count=c(n_levels_C,1)), ""))
  PRES_B_QC = unlist(str_split(ncvar_get(filenc_C, "PRES_QC", 
                                         start=c(1,id_prof_B), count=c(n_levels_C,1)), ""))
...
get_profile_match <- function(file_name, param_name, path_to_netcdf, PROFILE_DATE, method="night",
                              drift_A=0, drift_C=0, drift_Q=0, material="PEEK",
                              core_file_name=NULL) {
  
  match = get_Ts_match(path_to_netcdf=path_to_netcdf, file_name=file_name, PARAM_NAME=param_name,
                       material, core_file_name)
  ...
  if (method == "night") {  
    ...
    return(list("MATCH"=MATCH, "MATCH_Ts"=MATCH_Ts, "MATCH_date"=MATCH_date, 
                "MATCH_name"=MATCH_name, "MATCH_pres"=MATCH_pres))
  ...
  if (method == "day") {
    ...
    if (length(match_not_na) > 4) {  
      ...
      if (any(signif, na.rm=T)) {
        ...
        return(list("MATCH"=MATCH, "MATCH_Ts"=MATCH_Ts, "MATCH_date"=MATCH_date, 
                    "MATCH_name"=MATCH_name, "MATCH_darkmed"=MATCH_darkmed, 
                    "MATCH_which"=subsel_dark, "MATCH_pres"=MATCH_pres))
      }
    }
    return(list("MATCH"=NULL, "MATCH_Ts"=NULL, "MATCH_date"=NULL, "MATCH_name"=NULL, 
                "MATCH_darkmed"=NULL, "MATCH_which"=NULL, "MATCH_pres"=NULL))
  }
