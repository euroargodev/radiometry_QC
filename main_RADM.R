...
my_menu <- function(choices, title=NULL) { # equivalent to menu() but compatible with non 
...
main_RADM <- function(WMO, index_ifremer, index_greylist, path_to_netcdf, n_cores=detectCores()) {
  ...
  choice = my_menu(title = paste0("What kind of radiometry sensor is used in this float ? ",
                                  "(0 to abandon and quit)"),
                   choices = c("OCR504 (PEEK)", "OCR504 (Aluminium)")
  )
  ...
  variables = as.character(index_ifremer$parameters) #retrieve the list of variables available in 
                                                     #each file
  ...
  subset = which(wod==WMO & substr(prof_id,14,14)!="D" 
                 & !is.na(prof_date) & grepl("DOWNWELLING_PAR", variables) )
  subset_date_miss = which( wod==WMO & substr(prof_id,14,14)!="D" & is.na(prof_date) 
                           & grepl("DOWNWELLING_PAR", variables) )
  subset_copy = which(wod==WMO 
                      & ( substr(prof_id,14,14)=="D" | !grepl("DOWNWELLING_PAR", variables) ) )
  
  subset_pos_miss = which( wod==WMO & substr(prof_id,14,14)!="D" & 
                          grepl("DOWNWELLING_PAR", variables) & (is.na(lat) | is.na(lon)) )
  ...
  PARAM_NAMES = c("DOWNWELLING_PAR", "DOWN_IRRADIANCE380", 
                  "DOWN_IRRADIANCE412", "DOWN_IRRADIANCE490")
  
  is_greylisted <- function(julian_day, WMO, PARAMETER_NAME) {
    relevant_lines = which(index_greylist$PLATFORM_CODE==WMO &
                           index_greylist$PARAMETER_NAME==PARAMETER_NAME)
  ...
  drift_dataf = data.frame("PARAM"=DRIFT, "PARAM_Ts"=DRIFT_Ts, 
                           "PARAM_date"=DRIFT_date, "PARAM_name"=DRIFT_name)
  ...
  DRIFT_MATCH = mcmapply(get_profile_match, file_name=files_drift_PARALLEL, 
                         param_name=PARAM_NAMES_drift_PARALLEL, path_to_netcdf=path_to_netcdf, 
  ...
  for (param_name in PARAM_NAMES) {
    ...
    is_drift_outlier[param_axis] = (drift_dataf$PARAM[param_axis] < outliers_lim[1] |
                                    drift_dataf$PARAM[param_axis] > outliers_lim[2])
  ...
  repeat { # loop until the user finds an adequate fit or quits
    ...
    for (i in 1:4) {
      ...
      if (do_quadratic_fit[i]) {
        fit_ABC = lm(PARAM ~ PARAM_Ts + PARAM_date + PARAM_date_squared, data=drift_dataf, 
                     subset=subset_fit)
      ...
      drift_dataf_5C$PARAM[subset_PAR] = drift_dataf_5C$PARAM[subset_PAR] -
                                         B_axis_drift[i] * (drift_dataf_5C$PARAM_Ts[subset_PAR] - 5)
    }
    for (i in 1:4) {
      ### Alternative drift
      subset_PAR = which(DRIFT_dataf$PARAM_name == PARAM_NAMES[i])
    ...
    range_time = seq(min(c(drift_dataf$PARAM_date[good_drift], date_list)), 
                     max(c(drift_dataf$PARAM_date[good_drift], date_list)), length.out=100 )
    
    data_fit_drift = data.frame(PARAM_name = rep(PARAM_NAMES, each=2),
    ...
    if ( dim(drift_dataf)[1] != 0 ) {
      ...
      png(filename=plot_name, width=800, height=600)
      plot(g5_fit)
      dev.off()
  ...
  NIGHT_MATCH = mcmapply(get_profile_match, file_name=files_night_PARALLEL, 
                         param_name=PARAM_NAMES_night_PARALLEL, PROFILE_DATE=date_night_PARALLEL, 
                         drift_A=A_axis_drift_night_PARALLEL, drift_C=C_axis_drift_night_PARALLEL,
                         drift_Q=Q_axis_drift_night_PARALLEL, mc.cores=n_cores, USE.NAMES=FALSE, 
                         MoreArgs=list(path_to_netcdf=path_to_netcdf, material=sensor_type))
  DAY_MATCH = mcmapply(get_profile_match, file_name=files_day_PARALLEL, 
                       param_name=PARAM_NAMES_day_PARALLEL, PROFILE_DATE=date_day_PARALLEL, 
                       drift_A=A_axis_drift_day_PARALLEL, drift_C=C_axis_drift_day_PARALLEL, 
                       drift_Q=Q_axis_drift_day_PARALLEL, mc.cores=n_cores, USE.NAMES=FALSE, 
                       MoreArgs=list(method="day", path_to_netcdf=path_to_netcdf, 
                                     material=sensor_type))
  
  cat("DONE\nFlagging greylisted profiles and outliers in day profiles...")
  
  ### find dark median outliers in day profiles
  all_param_names = unlist(lapply(DAY_MATCH[4,], `[[`, 1)) 
                    # extract the first element of each profile
  ...
  for (param_name in PARAM_NAMES) {
    ...
    is_dark_outlier[param_axis] = (all_dark_median[param_axis] < outliers_lim[1] |
                                   all_dark_median[param_axis] > outliers_lim[2])
  }
  ...
  if (length(NIGHT_MATCH) == 0) { 
    ...
    PAR_dataf = data.frame("PARAM"=unlist(NIGHT_MATCH[1,]), 
                           "PARAM_Ts"=unlist(NIGHT_MATCH[2,]), 
                           "PARAM_date"=unlist(NIGHT_MATCH[3,]), 
                           "PARAM_name"=unlist(NIGHT_MATCH[4,]),
  ...
  TEST_MATCH = mcmapply(get_profile_match, file_name=files_drift_PARALLEL, 
                        param_name=PARAM_NAMES_drift_PARALLEL, path_to_netcdf=path_to_netcdf, 
                        PROFILE_DATE=date_drift_PARALLEL, mc.cores=n_cores, USE.NAMES=FALSE, 
  ...
  repeat {
    ...
    for (i in 1:length(PARAM_NAMES)) {
      ...
      run_min = running(fit_param, fun=min, width=51, pad=TRUE, align="center")
      run_min = run_min[26:(length(fit_param)+25)]
    ...
    g1 = ggplot(na.omit(PAR_dataf), aes(x=PARAM_Ts, y=PARAM, color=PARAM_pres, group=PARAM_name)) +
      ...
      facet_wrap(~PARAM_name, scale="free_y", 
                 labeller = labeller(PARAM_name = as_labeller(irr_labels, default=label_parsed))) +
      theme_bw() +
      labs(x="Sensor temperature (°C)", 
           y=TeX("$E_d(\\lambda)$ ($W\\, m^{-2}\\, nm^{-1}$) ; 
                   PAR ($\\mu mol\\, m^{-2}\\, s^{-1}$)"), 
           colour="Pressure (dbar)")#, title="Data from night profiles")
    g1_day = ggplot(na.omit(DAY_dataf), 
                    aes(x=PARAM_Ts, y=PARAM, color=PARAM_pres, group=PARAM_name)) +
    ...
    while(dev.cur() > 1) {dev.off()} # clear all windows
  ...
  if ( length(prof_num_date_miss) != 0 ) {
    ...
    if ( choice == 0 ) {
      ...
      for (i in 1:length(prof_num_date_miss)) {
        ...
        date_prev = date_list[which(as.numeric(substr(prof_id[subset], 11, 13)) 
                                    == (as.numeric(prof_num_date_miss[1]) - 1) )]
        date_next = date_list[which(as.numeric(substr(prof_id[subset], 11, 13)) 
                                    == (as.numeric(prof_num_date_miss[1]) + 1) )]
        
        question2 = c("For profile ", prof_num_date_miss[i], " :\n  Date of preceding profile : ", 
                      NA, "\n  Date of following profile : ", NA, 
                      "\nWhat should be done with this profile ? 
                        (If any date is unknown, do not use it)")
  ...
  if ( length(subset_pos_miss) != 0 ) {
    cat(paste0("Profiles with missing position : ", 
               paste(substr(prof_id[subset_pos_miss], 11, 13), collapse = ", "), "\n")
    )
  } else {
  ...
  corr_file <- function(file_name, PROFILE_DATE, path_to_netcdf, just_copy=FALSE) {
    ...
    file_name_out = paste(path_sep[1], path_sep[2], path_sep[3], "RADM/RADM_profiles", path_sep[4],
                          sep="/")
    ...
    for (i in 1:length(PARAM_NAMES)) {
      ...
      matchup = get_Ts_match(file_name=file_name, path_to_netcdf=path_to_netcdf, 
                             PARAM_NAME=PARAM_NAMES[i])
      
      match_day = get_profile_match(file_name=file_name, param_name=PARAM_NAMES[i], 
                                    path_to_netcdf=path_to_netcdf, PROFILE_DATE=PROFILE_DATE, 
                                    method="day", drift_A=A_axis_drift_corr[i], 
      ...
      corr_qc[match_day$MATCH_which] = rep(as.character(best_QC), length(match_day$MATCH_which)) 
                                       #if which==NULL --> no change
      ...
      param_undrifted = as.numeric( matchup$PARAM - A_axis_drift_corr[i] 
                                   - C_axis_drift_corr[i] * PROFILE_DATE
      ...
      scientific_coefficient = paste0("A = ", signif(A_total,4),", B = ", signif(B_axis_corr[i],4),
                                      ", C = ", signif(C_axis_drift_corr[i],4))
      if (Q_axis_drift_corr[i] != 0) {
        scientific_coefficient = paste0(scientific_coefficient, ", Q = ", 
                                        signif(Q_axis_drift_corr[i],4))
      }
      
      ### Scientific equation
      
      scientific_equation = paste0(PARAM_NAMES[i], "_ADJUSTED = ", PARAM_NAMES[i], 
                                   " - A - B*SENSOR_TEMP - C*JULD")
      if (Q_axis_drift_corr[i] != 0) {
      ...
      exit[i] = write_DM(file_out=full_file_name_out, param_name=PARAM_NAMES[i], DATE=DATE, 
                         scientific_comment=scientific_comment, 
                         scientific_coefficient=scientific_coefficient, 
                         scientific_equation=scientific_equation, 
                         comment_dmqc_operator_PRIMARY=comment_dmqc_operator_PRIMARY, 
                         comment_dmqc_operator_PARAM=comment_dmqc_operator_PARAM, 
  ...
  corr_all = mcmapply(corr_file, file_name=files_list, PROFILE_DATE=date_list, mc.cores=n_cores, 
                      SIMPLIFY=FALSE, MoreArgs=list(path_to_netcdf=path_to_netcdf))
  
  if ( length(subset_date_miss) != 0 ) {
    corr_date_miss = mcmapply(corr_file, file_name=files[subset_date_miss], 
                              PROFILE_DATE=date_list_missing, mc.cores=n_cores, SIMPLIFY=FALSE,
